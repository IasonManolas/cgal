...
private:
  ...
  */
  ...
  static void run_unordered(std::vector<Element_type>& candidates,
                            Operation& op, C3t3& c3t3)
  {
    // No shuffle. It was introduced to spread threads over the mesh and does
    // not pay for itself: the candidates are faster in the order
    // get_elements() produces them, and the gain is largest on the heavy
    // meshes. Same work, better locality.
    //
    // Reseeding from random_device on every call also made every run remesh a
    // different sequence, so no two runs were comparable. Without it a
