...
template <typename C3t3, typename CellSelector, typename Visitor> class EdgeFlipOperationBase
{
protected:
...
private:
  bool execute_internal_edge_flip(const ElementType& e, C3t3& c3t3) {
    ...
    edge_flip_logger.log("REFACTORED: Processing internal edge #", internal_edge_counter++, " from vertex ",
                         vp.first->point(), " to vertex ", vp.second->point());
#endif
...
private:
  bool execute_boundary_edge_flip(const ElementType& e, C3t3& c3t3) {
    ...
    if(show_boundary_debug && boundary_edge_counter < 5000) {
      edge_flip_logger.log("REFACTORED: Processing boundary edge #", boundary_edge_counter, " from vertex ",
                           vh0->point(), " to vertex ", vh1->point());
    }

    if(boundary_edge_counter == 4999)
      show_boundary_debug = false; // Stop after first 5000 edges (0-4999)
        ...
        if(db == VALID_FLIP) {
          ...
          if(show_boundary_debug && boundary_edge_counter - 1 < 5000) {
            edge_flip_logger.log("REFACTORED: Successfully flipped boundary edge #", boundary_edge_counter - 1,
                                 " from vertex ", vh0->point(), " to vertex ", vh1->point());
          }
#endif
          return true;
        }
