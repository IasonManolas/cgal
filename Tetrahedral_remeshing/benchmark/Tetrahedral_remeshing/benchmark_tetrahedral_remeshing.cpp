...
// Per-phase wall-clock totals (split/collapse/flip/smooth) printed once at the
// end. Enable to find which phase a change actually moved.
//#define CGAL_TETRAHEDRAL_REMESHING_PHASE_TIMINGS

// Must come before anything else so the malloc replacement is linked in.
#ifdef CGAL_TR_USE_TBBMALLOC_PROXY
...
int main(int argc, char** argv) {
  ...
  // Output-identity gate. CGAL_TR_DUMP_MESH=<path> writes the remeshed
  // triangulation in MEDIT form so two builds can be compared with cmp(1).
  // Quality metrics are a lossy summary -- two different meshes can share a
  // cell count -- so an optimisation only counts as output-identical when the
  // dumped files match byte for byte. Harness-only: never part of a patch.
  if(const char* const dump = std::getenv("CGAL_TR_DUMP_MESH"))
  {
    std::ofstream dos(dump);
    dos.precision(17);
    CGAL::IO::write_MEDIT(dos, tr, CGAL::parameters::all_cells(true));
  }
  ...
  // Virtual size. Kept because every earlier experiment recorded it, but it is
  // a poor measure of what the remesher uses: in a threaded build it is
  // dominated by the allocator's arena RESERVATIONS, which appear as worker
  // threads first touch the allocator. It is therefore bimodal -- runs of the
  // same binary on the same input differ by one arena, about 6 MB -- and the
  // step is absolute, so it is a larger fraction of a smaller footprint.
  append_metric_result(results_json, "Performance", "Memory", "Value", CGAL::Memory_sizer().virtual_size() >> 20);

  // Peak resident set size: the high-water mark of memory actually faulted in,
  // as the kernel accounted it. Monotone over the process lifetime, so unlike a
  // sampled figure it does not depend on when it is read, and it does not move
  // when the allocator reserves address space it never touches.
  {
    struct rusage ru;
    getrusage(RUSAGE_SELF, &ru);
    append_metric_result(results_json, "Performance", "Peak_RSS", "Value",
                         static_cast<long>(ru.ru_maxrss >> 10)); // KB -> MB
  }
